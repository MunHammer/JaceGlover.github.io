...
function getCode(): string {
  return workspace !== null ? Blockly.JavaScript.workspaceToCode(workspace) : "";
}
...
class Lesson {
  ...
  hint: string
  check: () => boolean
  constructor(chapter: string, chapterLabel: string, title: string, objective: string, hint: string, check: () => boolean) {
  ...
  9:
    new Lesson(
      ...
      () => {
        ...
        if (numMatch !== null && numMatch[1] !== undefined) return parseFloat(numMatch[1]) > 50;
        return false;
      },
    ),
  10:
...
function loadLesson() {
  ...
  const chapterBadgeWrap = document.getElementById("chapterBadgeWrap") as HTMLElement;
  const lessonTitle = document.getElementById("lessonTitle") as HTMLElement;
  const lessonObjective = document.getElementById("lessonObjective") as HTMLElement;
  ...
  if ([chapterBadgeWrap, l, lessonTitle, lessonObjective, lessonHint, outputDiv, progressBar, startBtn].some((element) => { return element === null || element === undefined })) throw new ReferenceError();
  // Actual stuff
...
startBtn.addEventListener("click", function() {
  ...
  if ([lessonIntro, lessonArea, toolbox].some((element) => { return element === null })) throw new ReferenceError();
  // The actual stuff
...
runBtn.addEventListener("click", function() {
  ...
  const progressText = document.getElementById("progressText") as HTMLElement;
  // Making sure elements exist
  if ([outputDiv, lessons[currentLesson], progressBar, progressText].some((thingToCheck) => { return thingToCheck === null || thingToCheck === undefined })) throw new ReferenceError();
...
function showCertificate() {
  ...
  // Making sure elements exist
  if ([certName,certDate, certificate].some((element) => {return element === null})) throw new ReferenceError();
